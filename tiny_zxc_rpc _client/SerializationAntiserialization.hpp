...
{
public:
	SerializationAntiserialization() { m_byteorder = LittleEndian; };
	~SerializationAntiserialization(){ };

	SerializationAntiserialization(StreamBuffer dev, int byteorder=LittleEndian){
...
public:
	...
	void getv(SerializationAntiserialization& ds, Tuple& t) {
		ds >> std::get<Id>(t);
	}
	...
	template<typename T>
	SerializationAntiserialization &operator >> (T& i){
		output_type(i); 
		return *this;
	}
...
template<typename T>
inline void SerializationAntiserialization::output_type(T& t)
{
	int len = sizeof(T);
	char* d = new char[len];
...
template<>
inline void SerializationAntiserialization::output_type(std::string& in)
{
...
template<typename T>
inline void SerializationAntiserialization::input_type(T t)
{
	int len = sizeof(T);
	char* d = new char[len];
...
template<>
inline void SerializationAntiserialization::input_type(std::string in)
{
	// 先存入字符串长度
...
template<>
inline void SerializationAntiserialization::input_type(const char* in)
{
	input_type<std::string>(std::string(in));
}

}
#endif   // SERIALIZATIONANTISERIALIZATION_H
